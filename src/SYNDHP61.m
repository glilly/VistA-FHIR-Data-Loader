...
PROBFDA ; build FDA array for Problems
 ...
 I '$D(LEX("P")) S RETSTA="-1^SNOMED CT code not found in Lexicon: "_$G(DHPSCT) Q
...
DXICDCS(SYS) ; FHIR CodeableConcept system to ICDEX coding system: 1=ICD-9, 30=ICD-10, 0=SNOMED path
 ...
 Q 0
 ;
ENCTUPD(RETSTA,DHPPAT,STARTDT,ENDDT,ENCPROV,CLINIC,SCTDX,SCTCPT,DXICDCS,HFACTORS,DHPVISIT) ;Encounter update
 ...
 S DHPICD=-1
 I $G(SCTDX)'="" D  ;
 .N ICDCS S ICDCS=+$G(DXICDCS)
 .I ICDCS=1!(ICDCS=30) D  ;
 ..N ICDTX S ICDTX=$$ICDDX^ICDEX(SCTDX,ICDCS)
 ..S DHPICD=+ICDTX
 ...
 S DHPCPT=$$MAP^SYNDHPMP("sct2cpt",SCTCPT)
 ; $$MAP^SYNDHPMP returns status^target-code, not a bare code.
 I +DHPCPT'=-1 D
 ...
 ; Mapped ICD: primary dx on PROVIDER (PXAI warns if PROVIDER DIAGNOSIS empty). Omit PROCEDURE DIAGNOSIS to avoid duplicate-primary POV vs DX/PL that PXAI may add.
 I $G(SCTDX)'="",+$G(DHPICD)'=-1 S ENCDATA("PROVIDER",1,"DIAGNOSIS")=DHPICD
 D ADDHF(.ENCDATA,.HFACTORS,APPTDATE,+$G(DHPVISIT))
 S ENCDATA("PROCEDURE",1,"PROCEDURE")=DHPCPT
 S ENCDATA("PROCEDURE",1,"QTY")=1
 ;
 K ZZERR,ZZERDESC,VISIT
 I +$G(DHPVISIT)>0 S VISIT=+DHPVISIT
 ...
 QUIT
 ;
ADDHF(ENCDATA,HFACTORS,APPTDATE,VISIT) ; Add explicit VistA Health Factors
 N CNT,HFIEN,HF,NAME,NOTE,SEV
 S CNT=0
 F  S CNT=$O(HFACTORS(CNT)) Q:+CNT=0  D
 . S NAME=$G(HFACTORS(CNT,"name"))
 . Q:NAME=""
 . S HFIEN=+$O(^AUTTHF("B",NAME,0))
 . Q:HFIEN<1
 . I +$G(VISIT)>0,$$HASHF(VISIT,HFIEN) Q
 . S HF=$O(ENCDATA("HEALTH FACTOR",""),-1)+1
 . S ENCDATA("HEALTH FACTOR",HF,"HEALTH FACTOR")=HFIEN
 . S ENCDATA("HEALTH FACTOR",HF,"EVENT D/T")=APPTDATE
 . S NOTE=$G(HFACTORS(CNT,"comment"))
 . I NOTE'="" S ENCDATA("HEALTH FACTOR",HF,"COMMENT")=NOTE
 . S SEV=$$SEV($G(HFACTORS(CNT,"severity")))
 . I SEV'="" S ENCDATA("HEALTH FACTOR",HF,"LEVEL/SEVERITY")=SEV
 Q
 ;
HASHF(VISIT,HFIEN) ; true if visit already has V Health Factor row
 N IND
 S IND=0
 F  S IND=$O(^AUPNVHF("AD",+$G(VISIT),IND)) Q:+IND=0  I +$P($G(^AUPNVHF(IND,0)),"^")=+$G(HFIEN) Q
 Q $S(+IND>0:1,1:0)
 ;
SEV(X) ; Map simple severity text/number to PCE code
 S X=$TR($G(X),"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 I X="" Q ""
 I X="M"!(X="MINIMAL") Q "M"
